...
exports.updateQuestion = catchAsync(async (req, res, next) => {
  ...
  // Handle option image updates and deletions for English images
  if (req.files.optionImageEnglish) {
    const oldOptionImagesEnglish = existingQuestion.optionImageEnglish || [];
    updateData.optionImageEnglish = req.files.optionImageEnglish.map((file, index) => {
      const oldImage = oldOptionImagesEnglish[index];
      if (oldImage) {
        deleteOldFiles(oldImage).catch((err) => {
          console.error(`Failed to delete old English option image ${index + 1}:`, err);
        });
      }
  ...
  // Handle option image updates and deletions for Hindi images
  if (req.files.optionImageHindi) {
    const oldOptionImagesHindi = existingQuestion.optionImageHindi || [];
    updateData.optionImageHindi = req.files.optionImageHindi.map((file, index) => {
      const oldImage = oldOptionImagesHindi[index];
      if (oldImage) {
        deleteOldFiles(oldImage).catch((err) => {
          console.error(`Failed to delete old Hindi option image ${index + 1}:`, err);
        });
      }
      return `${file.destination}/${file.filename}`;
    });
