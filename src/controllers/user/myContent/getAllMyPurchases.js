...
exports.getAllMyPurchases = catchAsync(async (req, res) => {
  ...
  if (type !== "Bookmark" || type === undefined) {
    ...
    for (const transaction of transactions) {
      // Check if the course has expired by comparing expiryAt with the current date
      if (transaction.expiryAt && new Date() > transaction.expiryAt) {
        continue; // Skip this transaction if expired
      }
      console.log(transaction,new Date(),"dladlfj")

      if (transaction.item) {
        const item = transaction.item.toObject();
        item.type = transaction.onModel;

        // Additional logic for TestSeries type
        if (transaction.onModel === "TestSeries") {
          const totalMockTests = await MockTest.countDocuments({
            testSeries: transaction.item._id,
          });
